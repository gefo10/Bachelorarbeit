...
public:
...

    {
        ...
        glfwSetKeyCallback(win, [](GLFWwindow* w, int key, int scancode, int action, int mods)
            {
                auto s = (window*)glfwGetWindowUserPointer(w);
                if (0 == action || action == GLFW_PRESS) // on key release
                {
                    s->on_key_release(key);
                }
            });
...
struct glfw_state {
    glfw_state(float yaw = 15.0, float pitch = 15.0) :iter(0), yaw(yaw), pitch(pitch), last_x(0.0), last_y(0.0),
...
void load_OBJ(std::string path,std::vector<glm::vec3>& out_vertices, std::vector<glm::vec2>& out_uvs, std::vector<glm::vec3>& out_normals)
{
    //result variables 
   ...
   // std::vector<glm::vec2> out_uvs;
    ...
    //variable to store line by line from the obj file
    std::string line;
    using namespace std;

    while(std::getline(file,line)) {
       //use a stream to separate words
       istringstream ss(line);

       string word;
       ss >> word;
       if(word.compare("v") == 0) {
           //the row represents a vertex
           glm::vec3 vertex;
           //read the xyz coordinates and parse them into the vec3 vertex
           ss >> vertex.x >> vertex.y >> vertex.y;
           
           //save vertex data
           temp_vertices.push_back(vertex);
       } else if (word.compare("vt") == 0) {
           //the row represents texture coord of one vertex
           glm::vec2 uv;
           ss >> uv.x >> uv.y;
           ...
           //the row represents the normal of one vertex
           glm::vec3 normal;
           ...
           //save all 9 values (but first clear the string to be clean)
           unsigned int vertexIndex[3],uvIndex[3],normalIndex[3];
           vector<string> values;
           boost::replace_first(line,"f","");
           boost::trim(line);
           boost::replace_all(line,"/"," ");
           istringstream ss_new(line);
           ss_new >> vertexIndex[0] >> vertexIndex[1] >> vertexIndex[2] >> uvIndex[0] >> uvIndex[1] >> uvIndex[2] >> normalIndex[0] >> normalIndex[1] >> normalIndex[2];
           for( int i = 0; i < 3; i ++) {
    ...
    for(unsigned int i = 0; i < vertexIndices.size(); i++) {
        ...
        //c++ starts at 0 but OBJ indexing starts at 1
        glm::vec3 vertex = temp_vertices[vertexIndex-1];
        out_vertices.push_back(vertex);

    }
    //same for uv and normals
